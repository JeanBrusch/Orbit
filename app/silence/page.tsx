...
interface LeadCardState {
  status: "idle" | "analyzing" | "generating" | "ready" | "sending" | "sent" | "error" | "low_signal";
    ...
    try {
      ...
      if (analysisRes.status === 422) {
        ...
        setState(s => ({ 
          ...
          status: "low_signal", 
          error: err.error,
          silenceAnalysis: { signals: err.signals, reason: err.governance_reason } 
        }));
        return;
      }
        ...
        <AnimatePresence mode="wait">
            ...
            >
              <div className="relative p-5 rounded-2xl bg-white/[0.02] border border-white/5 overflow-hidden">
                <div className="absolute top-1 right-1 flex items-center gap-2">
                    <div className={`px-2 py-0.5 rounded text-[7px] font-bold uppercase tracking-wider ${state.silenceAnalysis?.model_used === 'gpt-4o' ? 'bg-purple-500/20 text-purple-400 border border-purple-500/20' : 'bg-blue-500/20 text-blue-400 border border-blue-500/20'}`}>
                        {state.silenceAnalysis?.model_used === 'gpt-4o' ? 'Clinical 4o' : 'Mini'}
                    </div>
                ...
                <div className="flex flex-col gap-1 mb-4">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-blue-400 flex items-center gap-2">
                        Objetivo: <span className="text-white/60">{state.reengagement.objective?.replace(/_/g, " ").toUpperCase() || "RECONEXÃO"}</span>
                    </span>
                    {state.silenceAnalysis?.central_conflict && (
                        <span className="text-[9px] font-medium text-amber-400/80 bg-amber-400/5 px-2 py-0.5 rounded border border-amber-400/10 w-fit">
                            Conflict: {state.silenceAnalysis.central_conflict}
                        </span>
                    )}
                </div>
                ...
                {isEditing ? (
                    <textarea 
                        ...
                        className="w-full bg-transparent border-none focus:ring-0 text-sm font-light text-white/80 leading-relaxed resize-none h-24 p-0 shadow-none border-0"
                    />
                ) : (
                ...
                {state.silenceAnalysis?.mirroring_profile && (
                    <div className="mt-4 flex items-center gap-1.5 text-[8px] font-mono text-white/20 uppercase tracking-widest border-t border-white/5 pt-3">
                        <Waves className="w-2.5 h-2.5" />
                        Tone: {state.silenceAnalysis.mirroring_profile}
                    </div>
                )}
              </div>
          ...
          {state.status === "low_signal" && (
            <motion.div 
                ...
                animate={{ scale: 1, opacity: 1 }}
                className="w-full p-5 bg-white/[0.02] border border-white/5 rounded-2xl space-y-4"
            >
              <div className="flex items-center gap-3">
                  <ShieldCheck className="w-5 h-5 text-amber-500" />
                  <div>
                      <p className="text-[10px] font-bold text-amber-500 uppercase tracking-widest">Governança: Sinal Insuficiente</p>
                      <p className="text-[11px] text-white/60">Este lead não possui dados suficientes para uma análise clínica segura.</p>
                  </div>
              </div>
              
              <div className="grid grid-cols-3 gap-2">
                  <div className="p-2 rounded-xl bg-white/5 border border-white/5 flex flex-col items-center">
                    <span className="text-[10px] text-white/30 uppercase">Msgs</span>
                    <span className={`text-sm font-bold ${state.silenceAnalysis?.signals.messages > 1 ? 'text-emerald-400' : 'text-white/40'}`}>
                        {state.silenceAnalysis?.signals.messages}
                    </span>
                  </div>
                  <div className="p-2 rounded-xl bg-white/5 border border-white/5 flex flex-col items-center">
                    <span className="text-[10px] text-white/30 uppercase">Cliques</span>
                    <span className={`text-sm font-bold ${state.silenceAnalysis?.signals.interactions > 0 ? 'text-emerald-400' : 'text-white/40'}`}>
                        {state.silenceAnalysis?.signals.interactions}
                    </span>
                  </div>
                  <div className="p-2 rounded-xl bg-white/5 border border-white/5 flex flex-col items-center">
                    <span className="text-[10px] text-white/30 uppercase">Memórias</span>
                    <span className={`text-sm font-bold ${state.silenceAnalysis?.signals.memories > 0 ? 'text-emerald-400' : 'text-white/40'}`}>
                        {state.silenceAnalysis?.signals.memories}
                    </span>
                  </div>
              </div>

              <button onClick={() => setState(s => ({ ...s, status: "idle" }))} className="w-full py-3 bg-white/5 hover:bg-white/10 text-white/60 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all">
                  Voltar para Fila
              </button>
            </motion.div>
          )}
