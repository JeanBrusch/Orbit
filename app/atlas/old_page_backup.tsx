...
import { 
  ...
  MoreHorizontal, Heart, Share2, BrainCircuit, TrendingUp, X, MapPin, Sparkles, ArrowRight, Loader2, Plus, Link as LinkIcon, Trash2, Mic, Check
} from "lucide-react"
...
export default function AtlasPage() {
  ...
  const [selectedLeadForOffer, setSelectedLeadForOffer] = useState<MatchResult | null>(null)
  ...
  // Timeline State
  const [propertyTimeline, setPropertyTimeline] = useState<any[]>([])
  const [activePropertyTab, setActivePropertyTab] = useState<"intel" | "history">("intel")
  const [isAddingEvent, setIsAddingEvent] = useState(false)
  const [newEvent, setNewEvent] = useState({ type: 'visita', desc: '' })
  ...
  useEffect(() => {
    ...
    // Fetch History
    fetch(`/api/atlas/property-history?propertyId=${selectedProperty.id}`)
      .then(r => r.json())
      .then(data => setPropertyTimeline(data.history || []))
      .catch(e => console.error("History fetch failed", e))

  }, [selectedProperty?.id, matchCache])

  const handleAddTimelineEvent = async () => {
    if (!selectedProperty || !newEvent.desc) return
    try {
      const res = await fetch('/api/atlas/property-history', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        setPropertyTimeline(prev => [event, ...prev])
        setIsAddingEvent(false)
        setNewEvent({ type: 'visita', desc: '' })
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#0a0907] text-[#f1f5f9]" style={{ cursor: placingPropertyId ? "crosshair" : "default", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0907] via-transparent to-[#0a0907]/30 pointer-events-none -z-10 dark:block hidden" />
      </div>
      ...
      <header className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 px-6 py-2 ${glass} rounded-full w-fit max-w-[95%] shadow-[0_20px_40px_rgba(0,0,0,0.5)]`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 rounded-lg text-xs font-bold hover:bg-indigo-500 hover:text-white transition-all shrink-0"
          >
            <Mic className="w-3.5 h-3.5" />
            <span className="hidden md:inline">Cadastro Cognitivo</span>
          </button>
              ...
              <form onSubmit={handleConfirmIngest} className="space-y-4 animate-in fade-in zoom-in duration-300">
                <div className="flex gap-4 items-center p-3 rounded-lg bg-zinc-900/40 border border-[#d4af35]/10">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-0.5">Imagem do Imóvel (URL)</p>
                    <input 
                      type="text" 
                      value={scrapedData.image}
                      onChange={(e) => setScrapedData({...scrapedData, image: e.target.value})}
                      className="w-full bg-transparent border-none p-0 text-sm font-semibold focus:ring-0 text-[#d4af35]" 
                      placeholder="URL da Imagem..." 
                    />
                    <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mt-1.5 mb-0.5">Scraped Title</p>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                  ...
                  <div>
                     <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1">Localização/Condo</label>
                    <input
                      type="text"
                ...
                <div className="p-3 rounded-lg bg-indigo-500/5 border border-indigo-500/10 space-y-2">
                  <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest flex items-center gap-1.5">
                    <Sparkles size={10} /> Extração Cognitiva
                  </p>
                  <div className="grid grid-cols-3 gap-2">
                    <div className="text-center p-1.5 rounded bg-black/20">
                      <p className="text-[8px] text-zinc-500 uppercase">Quartos</p>
                      <p className="text-xs font-bold text-white">{scrapedData.bedrooms || '-'}</p>
                    </div>
                    <div className="text-center p-1.5 rounded bg-black/20">
                      <p className="text-[8px] text-zinc-500 uppercase">Suítes</p>
                      <p className="text-xs font-bold text-white">{scrapedData.suites || '-'}</p>
                    </div>
                    <div className="text-center p-1.5 rounded bg-black/20">
                      <p className="text-[8px] text-zinc-500 uppercase">Área (m²)</p>
                      <p className="text-xs font-bold text-white">{scrapedData.area_privativa || '-'}</p>
                    </div>
                  </div>
                </div>
      ...
      <AnimatePresence>
        {selectedProperty && (
          ...
          >
            ...
            {/* Tab Switcher */}
            <div className="px-5 pt-4 flex gap-4 border-b border-[#d4af35]/10">
              <button 
                onClick={() => setActivePropertyTab('intel')}
                className={`pb-2 text-[10px] font-bold uppercase tracking-widest transition-all ${activePropertyTab === 'intel' ? 'text-[#d4af35] border-b-2 border-[#d4af35]' : 'text-slate-500'}`}
              >
                Inteligência
              </button>
              <button 
                onClick={() => setActivePropertyTab('history')}
                className={`pb-2 text-[10px] font-bold uppercase tracking-widest transition-all ${activePropertyTab === 'history' ? 'text-[#d4af35] border-b-2 border-[#d4af35]' : 'text-slate-500'}`}
              >
                Histórico ({propertyTimeline.length})
              </button>
            </div>

            {/* Details & Intelligence Area */}
            <div className="p-5 flex-1 overflow-y-auto space-y-6 custom-scrollbar">
              {activePropertyTab === 'intel' ? (
                <>
              ...
              <div className="bg-[#d4af35]/5 border border-[#d4af35]/20 rounded-xl p-4 relative overflow-hidden">
                  ...
                  <div className="space-y-2">
                    {matches.map(m => (
                      <div 
                        key={m.id} 
                        onClick={() => {
                          setSelectedLeadForOffer(m)
                          setIsOfferModalOpen(true)
                        }}
                        className="flex items-center gap-3 bg-[#0a0907]/60 border border-[#d4af35]/10 p-2 rounded-lg group hover:border-[#d4af35]/40 transition-colors cursor-pointer"
                      >
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center bg-[#d4af35]/5 border border-[#d4af35]/10 p-3 rounded-lg">
                    <span className="text-[10px] font-bold text-slate-300 uppercase">Ações do Imóvel</span>
                    <button 
                      onClick={() => setIsAddingEvent(true)}
                      className="text-[9px] font-bold bg-[#d4af35]/20 text-[#d4af35] px-2 py-1 rounded hover:bg-[#d4af35]/30 transition-colors"
                    >
                      + Novo Evento
                    </button>
                  </div>
                  ...
                  {isAddingEvent && (
                    <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="p-3 bg-white/5 border border-white/10 rounded-lg space-y-2">
                      <select 
                        ...
                        className="w-full bg-zinc-900 border-white/10 rounded p-1.5 text-[10px] text-white focus:outline-none focus:ring-1 focus:ring-[#d4af35]"
                      >
                        <option value="visita">Visita Realizada</option>
                        <option value="proposta">Proposta Recebida</option>
                        <option value="rebaixamento">Rebaixamento de Preço</option>
                        <option value="feedback">Feedback Negativo</option>
                        <option value="venda">Vendido / Reservado</option>
                      </select>
                      <textarea 
                        value={newEvent.desc}
                        onChange={(e) => setNewEvent({ ...newEvent, desc: e.target.value })}
                        placeholder="Descreva o evento..."
                        className="w-full bg-zinc-900 border-white/10 rounded p-2 text-[10px] text-white h-16 focus:outline-none focus:ring-1 focus:ring-[#d4af35]"
                      />
                      <div className="flex gap-2">
                        <button onClick={() => setIsAddingEvent(false)} className="flex-1 py-1.5 text-[10px] text-zinc-500 bg-white/5 rounded">Cancelar</button>
                        <button onClick={handleAddTimelineEvent} className="flex-1 py-1.5 text-[10px] text-white bg-indigo-600 rounded">Salvar</button>
                      </div>
                    </motion.div>
                  )}

                  <div className="space-y-6 relative ml-2 mt-4">
                    <div className="absolute left-0 top-2 bottom-2 w-px bg-[#d4af35]/20" />
                    
                    {propertyTimeline.map((ev, i) => (
                      <div key={ev.id} className="relative pl-6">
                        <div className="absolute left-[-4px] top-1.5 size-2 rounded-full bg-[#d4af35] shadow-[0_0_8px_#d4af35]" />
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[9px] font-bold text-[#d4af35] uppercase tracking-tighter">{ev.event_type}</span>
                          <span className="text-[9px] text-slate-500">{new Date(ev.event_date).toLocaleDateString('pt-BR')}</span>
                        </div>
                        <p className="text-[11px] text-slate-200 leading-relaxed italic">"{ev.description}"</p>
                      </div>
                    ))}
                    ...
                    {propertyTimeline.length === 0 && (
                      <div className="text-center py-8 opacity-20 flex flex-col items-center gap-2">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-widest">Sem Histórico</p>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <AnimatePresence>
        {isOfferModalOpen && selectedLeadForOffer && selectedProperty && (
          <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <VoiceIngestion 
              ...
              onDataExtracted={(data) => {
                setScrapedData({
                  ...
                  condo_name: data.neighborhood || data.city || "",
