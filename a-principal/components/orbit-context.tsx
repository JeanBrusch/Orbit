...
export function OrbitProvider({ children }: { children: ReactNode }) {
  ...
  const setLeadVisualState = useCallback((leadId: string, visualState: LeadVisualState | undefined) => {
    setLeadStates((prev) => {
      const existing = prev[leadId] || {
        id: leadId,
        internalState: "default" as LeadInternalState,
        isPriority: false,
        isMuted: false,
        cycles: [],
        memory: { notes: "", callLog: [], contactLog: [] },
      }
      return {
        ...prev,
        [leadId]: {
