...
function useStatusData(): StatusBarData {
  ...
  const pendingCount = Object.values(leadStates).filter(
    (s) => s.memory?.followUpDue && new Date(s.memory.followUpDue) <= new Date()
  ).length

  // Derive ritmo based on activity
  let ritmo: RitmoSistema = "Estável"
  if (pendingCount > 3) ritmo = "Acumulando"
  if (adminLeadCount === 0 && pendingCount === 0) ritmo = "Pausado"

  // Derive pendencias label
  let pendenciasLabel = "Nenhuma pendência"
  if (pendingCount === 1) pendenciasLabel = "1 retorno"
  else if (pendingCount > 1) pendenciasLabel = `${pendingCount} aguardando`
  ...
  if (totalActive > 8 || pendingCount > 2) estadoCognitivo = "Foco"
  if (totalActive > 12 || pendingCount > 5) estadoCognitivo = "Pesado"

  return {
    ritmo,
    pendencias: { count: pendingCount, label: pendenciasLabel },
